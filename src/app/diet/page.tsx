...
export const metadata: Metadata = buildMetadata({
  ...
  description:
    "40대~갱년기 엄마들을 위한 기초대사 회복 다이어트 한약 매일감비환. 2026년 기준 7만건 이상 처방. 알약 개수로 1~8단계 복용량 조절, 감량기와 요요방지기를 나눈 처방. 1달 11만원~, 비대면 전국 처방.",
  path: "/diet",
...
const faqs = [
  {
    ...
    a: "네, 매일감비환에는 마황이 포함됩니다. 마황의 에페드린 성분이 교감신경을 자극해 체지방 분해를 촉진하는 것이 핵심 작용입니다. 다이어트 한약 부작용(두통·울렁거림·불면증·떨림·붕뜬 기분 등)은 모두 내 몸이 버틸 수 있는 용량보다 강하게 복용했을 때 나타납니다. 매일감비환은 1~8단계 복용량 미세 조정(알약 개수로 조절)을 통해 본인에게 맞는 단계를 찾습니다. 단계가 맞으면 불편한 반응을 줄이면서 감량기부터 요요방지기까지 이어갈 수 있습니다. 반응에는 개인차가 있어, 복용 전 상담에서 복용 중인 약과 기저질환을 함께 확인합니다.",
  },
  {
  ...
  },
  {
    ...
    a: "갱년기 이후에는 호르몬 변화와 기초대사량 하락이 겹쳐 복부에 살이 붙기 쉽습니다. 이 시기에는 식사량을 더 줄이는 것보다 대사를 회복시키는 방향으로 접근합니다. 여성호르몬제 등 복용 중인 약이 있으면 상담에서 알려 주세요. 개인 상태에 따라 처방 구성이 달라집니다.",
  },
  {
  ...
  {
    q: "얼마나 빨리 효과가 나타나나요?",
    a: "개인차가 큽니다. 보통 1개월 단위로 복용하며 상태를 확인하고, 기초대사량이 많이 떨어진 분일수록 초기에는 변화가 더디게 느껴질 수 있습니다. 체질·생활 패턴에 따라 달라지므로 상담에서 현재 상태를 보고 기간을 함께 정합니다.",
  },
  {
    q: "복용 중 식단·운동을 따로 해야 하나요?",
    a: "엄격한 식단 관리를 전제로 하는 처방은 아닙니다. 다만 끼니를 거르지 않는 것과 가벼운 걷기 정도는 함께 하시길 권합니다. 수면이 부족하거나 식사 시간이 불규칙하다면 그 부분도 상담에서 함께 살펴봅니다.",
  },
  {
    q: "요요가 생기지 않나요?",
...
const targetGroups = [
  ...
  {
    title: "주사 방식이 부담스러운 분",
    desc: "주 1회 주사가 부담스러워 다른 방법을 찾으시는 분. 복용량을 단계로 조절하는 방식이 맞는지 상담에서 확인합니다.",
  },
  {
    title: "수면 부족·불규칙 생활이라 식단 관리가 어려운 분",
    desc: "수면이 부족하고 식사 시간이 일정하지 않은 환경에서도 이어갈 수 있는 방식인지 함께 확인합니다.",
  },
  {
    title: "요요를 반복하는 분",
  ...
  return (
    <>
      ...
      <JsonLd id="schema-diet-product" data={{
        ...
        "@type": "Product",
        name: "매일감비환",
        description: "40대~갱년기 엄마들을 위한 기초대사 회복 다이어트 한약. 2026년 7만건 이상 처방. 알약 개수로 1~8단계 복용량 조절, 감량기 2~3개월과 요요방지기 6개월을 나눈 처방.",
          ...
          <div className="sn-reveal order-2 md:order-1">
            ...
            <div className="flex flex-wrap gap-3 mb-9">
              {[
                ...
                { num: "1~8단계", label: "복용량 미세 조절" },
                { num: "감량기+요요방지기", label: "기간을 나눈 처방" },
        ...
        <div className="mx-auto max-w-7xl px-5 md:px-8 lg:px-12 py-16 md:py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-3xl mx-auto mb-12">
            ...
            <div className="sn-reveal rounded-[22px] border border-white/[0.08] bg-white/[0.03] p-8 text-center" style={{ transitionDelay: "70ms" }}>
              <p className="font-serif text-5xl md:text-6xl text-white tracking-tight">
                1~8<span className="text-2xl align-top">단계</span>
              </p>
              <p className="text-sm text-white/50 mt-3">
복용량 미세 조절 <span className="text-white/30 text-xs">(알약 개수로 조절)</span>
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-10 border-t border-white/[0.07] pt-10">
            {[
              { num: "감량기", label: "2~3개월 설계" },
              { num: "요요방지기", label: "6개월 설계" },
      ...
      <section className="bg-[#F5F2EC] border-t border-black/[0.05]">
        <div className="mx-auto max-w-4xl px-5 md:px-8 py-20 md:py-24 text-center sn-reveal">
          ...
          <p className="text-sm text-[#525252] mb-8 leading-[1.75]">
            매일감비환 전용 홈페이지에서 복용 방법과 생활 관리 영상을 확인하실 수 있습니다.
          </p>
          <a
            href="https://mail100diet.com/"
            target="_blank"
