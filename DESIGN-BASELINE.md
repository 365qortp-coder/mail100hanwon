...
## ⚠ 리뉴얼 HTML에서 카피는 가져오지 않는다 (2026-09-18 추가)

`디자인리뉴얼\` 의 diet.html·index.html 에는 **의료광고법이 금지하는 문구가 그대로 남아 있다.**
디자인(레이아웃·색·간격)만 참고하고, 아래 종류의 문구는 절대 옮기지 않는다.

- 감량 수치 — "2달 평균 7.2kg", "51일 5.1kg 감량", "8.7kg 감량", "82일 4.6kg"
- 치료 결과 비율 — "89.9% 체지방 위주 감량", "후기 평균 48일"
- 환자 후기 — "실제 복용 후기" 섹션 전체, "실제 후기에서…"
- "부작용 없이" — 안전을 단정하는 표현
- 타 치료법 비교 — 마운자로·위고비를 "구역·구토·췌장염 우려"로 비교
- 슬러그에 박힌 것도 포함 — `마황-…-부작용없이-먹는법`

**옮기기 전에 반드시 검사한다.**

```
node scripts/check-site-copy.mjs src
```

`X` 가 하나라도 뜨면 배포하지 않는다. 실사이트에서는 2026-09-18에 전부 걷어냈다.
(같은 문구가 mail100diet.com 쪽에도 남아 있을 수 있다 — 별도 점검 필요)
